const { Trolly, StoreLocation } = require('../models');
...
class TrollyController {
  ...
  async updateTrolly(req, res) {
    try {
      const trolly = await Trolly.findOne({ where: { trolly_id: req.params.id } });
   ...
   */
  async getTrollyByRFID(req, res) {
    try {
      const trolly = await Trolly.findOne({
        where: { rfid_tag: req.params.rfidTag },
        include: [{ model: StoreLocation, as: 'currentLocation' }]
      });
      ...
      if (!trolly) {
        return res.status(404).json({
          success: false,
          message: 'Trolly not found with this RFID tag'
        });
      }
